function PhasedArrayGUI()
    ...
    % Reformatted for robustness (no line continuations).

    % Create Figure
    f = figure('Name', 'Phased Array Focal Law Calculator', 'NumberTitle', 'off', 'Position', [100, 100, 1200, 800], 'MenuBar', 'none', 'ToolBar', 'figure');

uicontrol('Style', 'text', 'String', 'Probe Settings', 'Position',...
          [ 20, 760, 200, 20 ], 'FontWeight', 'bold', 'HorizontalAlignment',...
          'left');

% -- -Helper Functions-- - function h = addParam(y, label, val, tag)
    uicontrol('Style', 'text', 'String', label, 'Position', [ 20, y, 140, 20 ],...
              'HorizontalAlignment', 'right');
h = uicontrol('Style', 'edit', 'String', num2str(val), 'Position',...
              [ 170, y, 100, 20 ], 'Tag', tag, 'BackgroundColor', 'white');
end

    function h = addCombo(y, label, items, tag)
        uicontrol('Style', 'text', 'String', label, 'Position',...
                  [ 20, y, 140, 20 ], 'HorizontalAlignment', 'right');
h = uicontrol('Style', 'popupmenu', 'String', items, 'Position',...
              [ 170, y, 100, 20 ], 'Tag', tag);
end
...
uicontrol('Style', 'text', 'String', 'Wedge Settings', 'Position',...
          [ 20, y, 200, 20 ], 'FontWeight', 'bold', 'HorizontalAlignment',...
          'left');
y = y - 30;
...
uicontrol('Style', 'text', 'String', 'Material Settings', 'Position',...
          [ 20, y, 200, 20 ], 'FontWeight', 'bold', 'HorizontalAlignment',...
          'left');
y = y - 30;
...
uicontrol('Style', 'text', 'String', 'Scan Settings', 'Position',...
          [ 20, y, 200, 20 ], 'FontWeight', 'bold', 'HorizontalAlignment',...
          'left');
y = y - 30;

h_focusMode = addCombo(y, 'Focus Type:',...
    {'Constant Depth', 'Vertical Line', 'Constant Sound Path'}, 'focusMode');
y = y - 30;
...
btn_calc = uicontrol('Style', 'pushbutton', 'String', 'Calculate', 'Position',...
                     [ 100, y, 150, 40 ], 'Callback', @runCalculation,...
...
% 2. Setup Objects probe = Probe(nEl, pPitch, 5e6);
...
% Wedge Polygon elX = elements( :, 1);
p1 = elements(1, :);
p2 = elements(end, :);

if abs (p2(1) - p1(1))...
  > 1e-9 m_slope = (p2(2) - p1(2)) / (p2(1) - p1(1));
...
patch(ax, [ wMinX, wMaxX, wMaxX, wMinX ] * 1000, [ 0, 0, zR, zL ] * 1000, 'c',...
      'FaceAlpha', 0.2, 'EdgeColor', 'none');

centerX = (elements(1, 1) + elements(end, 1)) / 2;
colors = jet(length(angles));

validCount = 0; % Counter for valid laws
...
for i = 1 : length(angles) 
    ...
    plot(ax, [ elements(1, 1), intPts(1, 1) ] * 1000,...
                     [ elements(1, 2), intPts(1, 2) ] * 1000, 'Color',...
                     c);
    plot(ax, [ intPts(1, 1), fx ] * 1000, [ intPts(1, 2), fz ] * 1000,...
         'Color', c);
